...
void loadImage(const char* imagePath, GLuint& textureID) {
    ...
    if (strstr(imagePath, ".pgm")) {
        // Convert PGM to temporary PNG using OpenCV
        cv::Mat pgmImage = cv::imread(imagePath, cv::IMREAD_GRAYSCALE);
        std::vector<int> compression_params;
        compression_params.push_back(cv::IMWRITE_PNG_COMPRESSION);
        compression_params.push_back(9);

        cv::imwrite("temp.png", pgmImage, compression_params);
...
void changeImages() {
    ...
    std::string imagePath2 = "capture/PGM/img_middle" + std::to_string(idx) + ".pgm";
...
int main(int argc, char** argv) {
    ...
    changeImages();  // Load the initial set of images
...
// g++ main.cpp $(pkg-config --cflags --libs opencv4) -lopencv_core -lopencv_highgui -lopencv_imgproc -lGL -lGLU -lglut -lSOIL && ./a.out
